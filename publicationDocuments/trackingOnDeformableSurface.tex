...
The basic problem we have is that we are given 5 values from the probe, $(x_{probe},y_{probe},\theta_{yaw},\theta_{pitch},\theta_{roll})$ and we need to translate that into 3 values for the virtual world, $(x_{virtual},y_{virtual},z_{virtual})$. Here the $x,y,z$ refer to displacements in those directions.  \\
\\
The probe gives us $x,y$ displacements, so we get the vector $(x,y,0)$ from the probe data. We use the orientation angles to get a rotation matrix and then apply the rotation matrix to that vector and obtain $(x,y,z)$ in the probe coordinate system. We then need to translate that to the virtual coordinate system. They are both orthogonal coordinate systems so an orthogonal matrix will allow us to transform between them. \\
\\
**MENTION SCALING**
\\
**MENTION ROTATION AND REFLECTION**
